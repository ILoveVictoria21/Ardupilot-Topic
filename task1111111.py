...
def main():
    ...
    # 顯示航點信息
    print(f"起始點: {START_LAT}, {START_LON}")
    print(f"航點一: {WP1_LAT}, {WP1_LON}")
    print(f"航點二: {WP2_LAT}, {WP2_LON}")
    print(f"航點三: {WP3_LAT}, {WP3_LON}")
    ...
    # 1. 起飛後在起始點懸停 10 秒
    # 為了確保位置準確，先導航到 START_LAT, START_LON
    print("前往起始點並懸停 10 秒...")
    ok_start = goto_wait(vehicle, START_LAT, START_LON, radius=ARRIVAL_RADIUS, timeout=180)
    if not ok_start:
        print("到達起始點超時,直接返航")
        do_qrtl_return_and_wait(vehicle)
        cleanup_after_mission(vehicle, modified_arm_check if modified_arm_check is not None else orig_arm_check)
    ...
    # 2. 飛向航點一
    print("前往航點一...")
    ok_wp1 = goto_wait(vehicle, WP1_LAT, WP1_LON, radius=ARRIVAL_RADIUS, timeout=180)
    if not ok_wp1:
        print("到達航點一超時,直接返航")
        do_qrtl_return_and_wait(vehicle)
        cleanup_after_mission(vehicle, modified_arm_check if modified_arm_check is not None else orig_arm_check)
    ...
    # 3. 飛向航點二
    print("前往航點二...")
    ok_wp2 = goto_wait(vehicle, WP2_LAT, WP2_LON, radius=ARRIVAL_RADIUS, timeout=180)
    if not ok_wp2:
        print("到達航點二超時,直接返航")
        do_qrtl_return_and_wait(vehicle)
        cleanup_after_mission(vehicle, modified_arm_check if modified_arm_check is not None else orig_arm_check)
    ...
    # 4. 飛向航點三
    print("前往航點三...")
    ok_wp3 = goto_wait(vehicle, WP3_LAT, WP3_LON, radius=ARRIVAL_RADIUS, timeout=180)
    if not ok_wp3:
        print("到達航點三超時,直接返航")
        do_qrtl_return_and_wait(vehicle)
        cleanup_after_mission(vehicle, modified_arm_check if modified_arm_check is not None else orig_arm_check)
    ...
    # 5. 再回到起始點
    print("返回起始點...")
    ok_return = goto_wait(vehicle, START_LAT, START_LON, radius=ARRIVAL_RADIUS, timeout=180)
    if not ok_return:
        print("返回起始點超時,直接返航")
        do_qrtl_return_and_wait(vehicle)
        cleanup_after_mission(vehicle, modified_arm_check if modified_arm_check is not None else orig_arm_check)
